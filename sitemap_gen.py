...
class HTMLLoad:
    ...
    def get(self, url, allow_redirects=False):
        ...
        try:
            resp = self.session.get(url, timeout=10, allow_redirects=allow_redirects)
...
class MyHTMLParser(HTMLParser):
    def __init__(
        self, pageMap, redirects, baseUrl, maxUrls, blockExtensions, robotParser
    ):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag.upper() == "A":
            ...
            for attr in attrs:
                # print("  attr: " + str(attr))
                if (attr[0].upper() == "REL") and (
                    attr[1].upper().find("NOFOLLOW") != -1
                ):
                    # We have discovered a nofollow, so we won't continue
                    return
                elif (attr[0].upper() == "HREF") and (
                    attr[1].upper().find("MAILTO:") == -1
                ):
            ...
            if self.hasBlockedExtension(url) or self.redirects.count(url) > 0:
                return
            if self.robotParser is not None and not self.robotParser.allowed(
                url, "sitemap_gen"
            ):
...
def parsePages(loader, startUrl, maxUrls, blockExtensions):
    ...
    while True:
        ...
        if page is None:
            ...
            if (
                urllib.parse.urlsplit(newUrl)[1] == server
                and newUrl not in pageMap
                and newUrl not in redirects
                and (robotParser is None or robotParser.allowed(newUrl, "sitemap_gen"))
            ):
                pageMap[newUrl] = EMPTY
        else:
            pageMap[url] = loader.date
            parser = MyHTMLParser(
                pageMap, redirects, url, maxUrls, blockExtensions, robotParser
            )
            try:
...
def main():
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            if (maxUrls < 0) or (maxUrls > 50000):
                sys.stderr.write(
                    "The maximum number of URLs must be between 1 and 50000\n"
                )
                return 1
    ...
    if not args:
        sys.stderr.write(
            "You must provide the starting URL.\nTry the -h option for help.\n"
        )
        return 1
